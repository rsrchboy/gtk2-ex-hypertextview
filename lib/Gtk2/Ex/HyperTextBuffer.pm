...
our %TAGS = (
    ...
    ,    # wrap_mode => 'none'], # Bug withwrap_mode !?
    Verbatim => [
        family        => 'monospace',
        wrap_mode     => 'none',
        'left-margin' => 40,
    ...
    head5 =>
        [ weight => PANGO_WEIGHT_BOLD, scale => 1.15, style => 'italic' ],
);
...
sub insert_blocks {
    ...
    my $mark   = $self->create_mark('insert-blocks-orig-insert',
        $self->get_iter_at_mark($insert), 1);
    ...
    for (@blocks) {
        unless (ref $_) {
            ...
            if (@{ $self->{edit_mode_tags} }) {
                my $end   = $self->get_iter_at_mark($self->get_insert);
                my $start = $end->copy;
...
sub get_parse_tree {
    ...
    EXTRACT_TAGS:
        ...
        if ($slice =~ /\x{fffc}/) {
            ...
            while ($slice =~ s/^(.*?)\x{fffc}//s) {    # Embeded image
                ...
                if ($pixbuf) {
                    ...
                    my $node
                        = ($$meta{type} and $$meta{type} eq 'checkbox')
                        ? 'checkbox'
                        : 'image';
        ...
        for ($iter->get_toggled_tags(1)) {    # opened tags
            my $link_data
                = Gtk2::Ex::HyperTextView->get_link_at_iter($iter, $_)
                if $_->{is_link};             # FIXME quick hack here
            my $tag
                = $_->{is_link}
                ? [ 'link', { to => $link_data } ]
    ...
    @$root = grep {
        defined($_)
            and (ref($_) and ($$_[0] ne 'image') and ($$_[0] ne 'checkbox'))
            ? (@$_ > 2)
            : length($_)
    } @$root;
...
sub _cut_or_copy {
    ...
    $clipboard->set_with_data(\&_clipboard_get, \&_clipboard_clear, $tree,
        @_targets)
        or return;
...
sub _clipboard_get {    # clipboard callback
    ...
    if ($selection->target->name eq 'ZIM_TREE') {
        ...
        $selection->set(Gtk2::Gdk::Atom->intern('ZIM_TREE', 0), 8, "$tree");
    }
    else {              # text
...
sub _dump_tree {    # TODO add "\n" around headings and support lists
    ...
    my $tree   = shift;
    my $string = '';
    $string .= ref($_) ? _dump_tree($_) : $_ for @$tree[ 2 .. $#$tree ];
    $string .= "\n" if $$tree[0] =~ /^head/;
...
sub paste_clipboard {
    ...
    if ($atom) {
        $clipboard->request_contents($atom, \&_clipboard_paste, $paste_data);
    }
    else {
        $clipboard->request_text(\&_clipboard_paste, $paste_data);
    }
}
